...
// 组件模版
const componentsTep = `import Taro, { Component } from '@tarojs/taro';
import { View } from '@tarojs/components';
import { connect } from '@tarojs/redux';
import './index.less';
@connect(({${dirName}}) => ({
...
export default class ${titleCase(dirName)}Item extends Component {
  
  render() {
    return (
      <View className="${dirName}-item">
        ${dirName}item
      </View>
    )
  }
...
fs.mkdirSync(`../../components/${dirName}Child`); // mkdir $1
process.chdir(`../../components/${dirName}Child`); // cd $1

fs.writeFileSync('index.js', componentsTep);
fs.writeFileSync('index.less', lessTep);
